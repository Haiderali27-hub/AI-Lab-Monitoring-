...
Another important characteristic of these requirements is that they reflect academic control as well as software capability. The purpose of the system is not only to automate routine work, but also to provide exam governance in a form that is transparent, auditable, and fair. The functional requirements of the proposed system are detailed in Table~\ref{tab:functionalrequirements}, outlining each identifier alongside its description in operational sequence.

The key quality attributes and non-functional requirements are summarized in Table~\ref{tab:nonfunctionalrequirements}.
...
Table~\ref{tab:functionalrequirements} provides the compact list of the system's functional expectations. However, each requirement becomes more meaningful when interpreted within the actual practical-exam environment. The following narrative expands the major functional areas so that the intended product behaviour is clearer.
...
The project time frame translates the Agile development plan into a structured schedule consisting of eight sprints. Each sprint lasts four weeks, allowing iterative development, review, and refinement while keeping the project aligned with academic deadlines. The schedule and major sprint activities are outlined in Table~\ref{tab:timeframe}. The schedule and major sprint activities are outlined in Table~\ref{tab:timeframe}.
...
The sprint-based schedule presented in Table~\ref{tab:timeframe} ensures continuous progress, regular supervisor feedback, and balanced development across all twelve modules of the proposed system. The final sprint focuses on integration and polishing so that the prototype is complete and academically presentable.
