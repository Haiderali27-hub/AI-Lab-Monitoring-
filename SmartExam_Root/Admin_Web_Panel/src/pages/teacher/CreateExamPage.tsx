...
interface QuestionForm {
  ...
  correctOptionIndex: number | null; // MCQ only
  referenceAnswer: string;           // Coding/Theory sample solution
}
...
export default function CreateExamPage() {
  ...
  // In edit mode, the prefetched assignment list must survive the section-students reload
  const [editAssignedIds, setEditAssignedIds] = useState<string[] | null>(null);
  ...
  // Edit mode: prefill the wizard from the existing exam
  useEffect(() => {
    if (!examId) return;
    examsApi.getById(examId)
      .then((exam: any) => {
        if (exam.status !== 'Scheduled') {
          setError(`This exam is already ${exam.status} and can no longer be edited.`);
          setPrefilling(false);
          return;
        }
        setTitle(exam.title ?? '');
        setInstructions(exam.instructions ?? '');
        setSelectedSectionId(exam.sectionId);
        const start = new Date(exam.startTime);
        setScheduledDate(start.toLocaleDateString('en-CA')); // YYYY-MM-DD (local)
        setStartTime(start.toTimeString().slice(0, 5));      // HH:mm (local)
        setDurationMinutes(exam.durationMinutes);
        try {
          const apps = JSON.parse(exam.allowedApps || '[]');
          setAllowedApps(Array.isArray(apps) ? apps.join(', ') : '');
        } catch { setAllowedApps(''); }
        setAiEvaluationEnabled(exam.aiEvaluationEnabled ?? true);
        setPlagiarismThreshold(exam.plagiarismThreshold ?? 70);
        const qs = [...(exam.questions ?? [])].sort((a: any, b: any) => a.orderIndex - b.orderIndex);
        ...
        setEditAssignedIds((exam.assignments ?? []).map((a: any) => a.userId));
        setPrefilling(false);
      })
      .catch(() => {
        setError('Failed to load the exam for editing.');
        setPrefilling(false);
      });
  }, [examId]);
  ...
  useEffect(() => {
    if (selectedSectionId) {
      examsApi.getSectionStudents(selectedSectionId)
        .then(data => {
          setStudents(data);
          if (editAssignedIds !== null) {
            // Edit mode: restore the exam's existing assignments (limited to enrolled students)
            const enrolled = new Set(data.map(s => s.userId));
            setAssignedStudentIds(editAssignedIds.filter(id => enrolled.has(id)));
            setEditAssignedIds(null);
          } else {
  ...
  const handleAddQuestion = () => {
    setQuestions(prev => [
      ...prev,
      { type: 'Coding', bodyText: '', marks: 10, testCases: [], options: [], correctOptionIndex: null, referenceAnswer: '' }
    ]);
  };

  // Switching type resets type-specific data so stale MCQ/test-case state can't leak
  const handleTypeChange = (index: number, newType: QuestionType) => {
    setQuestions(prev => prev.map((q, i) => {
      ...
      return {
        ...q,
        type: newType,
        testCases: newType === 'Mcq' ? [] : q.testCases,
        options: newType === 'Mcq' ? (q.options.length >= 2 ? q.options : ['', '']) : [],
        correctOptionIndex: newType === 'Mcq' ? q.correctOptionIndex : null,
        referenceAnswer: newType === 'Mcq' ? '' : q.referenceAnswer
      };
    }));
  };
  ...
  const handleRemoveOption = (qIndex: number, optIndex: number) => {
    setQuestions(prev => prev.map((q, i) => {
      ...
      if (correct !== null) {
        if (correct === optIndex) correct = null;
        else if (correct > optIndex) correct = correct - 1;
      }
  ...
  const handleCorrectOptionChange = (qIndex: number, optIndex: number) => {
    setQuestions(prev => prev.map((q, i) =>
      i === qIndex ? { ...q, correctOptionIndex: optIndex } : q
    ));
  };
  ...
  const validateStep = () => {
    ...
    if (step === 1) {
      ...
      for (let i = 0; i < questions.length; i++) {
        ...
        if (q.type === 'Mcq') {
          if (q.options.length < 2) return `Question ${i + 1} is an MCQ and needs at least 2 options.`;
          for (let j = 0; j < q.options.length; j++) {
            if (!q.options[j].trim()) return `Question ${i + 1} option ${j + 1} is empty.`;
          }
          if (q.correctOptionIndex === null) return `Question ${i + 1}: select the correct option.`;
          continue;
        }
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      questions: questions.map(q => ({
        ...
        bodyText: q.bodyText,
        marks: q.marks,
        testCases: q.type === 'Mcq' ? [] : q.testCases,
        options: q.type === 'Mcq' ? q.options : null,
        correctOptionIndex: q.type === 'Mcq' ? q.correctOptionIndex : null,
        referenceAnswer: q.type !== 'Mcq' && q.referenceAnswer.trim() ? q.referenceAnswer : null
      })),
    ...
    try {
      ...
      setError(err.response?.data?.message || `Failed to ${isEditMode ? 'update' : 'create'} exam. Please try again.`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AppLayout title={isEditMode ? 'Edit Exam' : 'Create New Exam'}>
                  ...
                  {questions.map((q, qIndex) => (
                    <div key={qIndex} className="bg-slate-50 border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                      ...
                      <div className="p-6 space-y-4 bg-white">
                          ...
                          <div className="mt-4 border border-slate-200 rounded-xl overflow-hidden bg-slate-50/50">
                            <div className="px-4 py-2 border-b border-slate-200 flex justify-between items-center bg-slate-50">
                              <span className="font-semibold text-[10px] text-slate-500 uppercase tracking-wider">
                                Options ({q.options.length}) — select the correct answer
                              </span>
                              <button
                                type="button"
                            ...
                            {q.options.length === 0 ? (
                              <div className="p-4 text-center text-xs text-slate-400 italic">No options yet. An MCQ needs at least 2 options.</div>
                            ) : (
                              <div className="divide-y divide-slate-100 bg-white">
                                {q.options.map((opt, optIndex) => (
                                  <div key={optIndex} className="flex items-center gap-3 px-4 py-2">
                                    <input
                                      type="radio"
                                      name={`correct-option-${qIndex}`}
                                      checked={q.correctOptionIndex === optIndex}
                                      onChange={() => handleCorrectOptionChange(qIndex, optIndex)}
                                      className="text-green-600 focus:ring-green-500/20"
                                      title="Mark as correct answer"
                                    />
                                    <span className="text-[10px] font-bold text-slate-400 w-5">{String.fromCharCode(65 + optIndex)}.</span>
                                    <input
                                      type="text"
                                      value={opt}
                                      onChange={e => handleOptionChange(qIndex, optIndex, e.target.value)}
                                      className="flex-1 bg-slate-50 border border-slate-200 rounded px-2 py-1 text-slate-800 text-xs"
                                      placeholder={`Option ${String.fromCharCode(65 + optIndex)}`}
                                    />
                                    {q.correctOptionIndex === optIndex && (
                                      <span className="px-2 py-0.5 bg-green-50 text-green-700 rounded-full text-[9px] font-bold uppercase">Correct</span>
                                    )}
                                    <button
                                      type="button"
                        ...
                        {/* Reference Answer (Coding / Theory) */}
                        {q.type !== 'Mcq' && (
                          <div>
                            <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">
                              REFERENCE ANSWER / SAMPLE SOLUTION <span className="text-slate-300 normal-case font-medium">(optional — never shown to students, used for grading review)</span>
                            </label>
                            <textarea
                              value={q.referenceAnswer}
                              onChange={e => handleQuestionChange(qIndex, 'referenceAnswer', e.target.value)}
                              className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500/10 focus:border-primary-500 outline-none transition-all text-slate-800 font-mono text-xs"
                              placeholder={q.type === 'Coding' ? 'Model solution code...' : 'Model answer / key points...'}
                              rows={2}
                            />
                          </div>
                        )}
