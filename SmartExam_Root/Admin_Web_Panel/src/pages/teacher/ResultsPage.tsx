...
// Parse the backend's MatchingSegments JSON string ([{ A, B }]) into typed pairs.
// Returns [] for missing/empty/malformed input so callers can show a safe fallback.
function parseMatchingSegments(raw?: string): { A: string; B: string }[] {
  if (!raw) return [];
  try {
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) return [];
    return parsed
      .filter((s: any) => s && (typeof s.A === 'string' || typeof s.B === 'string'))
      .map((s: any) => ({ A: s.A ?? '', B: s.B ?? '' }));
  } catch {
    return [];
  }
}
                  ...
                  plagiarism.map(flag => (
                    <div key={flag.plagId} className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                      ...
                      <div className="p-6">
                        ...
                        {(() => {
                          const segments = parseMatchingSegments(flag.matchingSegments);
                          if (segments.length === 0) {
                            // No stored segments — do not fabricate code. Honest note; the real
                            // similarity score and student names above remain fully live.
                            return (
                              <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 text-sm text-slate-500">
                                <span className="material-symbols-outlined text-slate-400 text-[20px]">info</span>
                                <span>Side-by-side highlighting uses stored match segments; detailed line highlighting is a FYP-2 enhancement.</span>
                              </div>
                            );
                          }
                          const codeA = segments.map(s => s.A).join('\n\n');
                          const codeB = segments.map(s => s.B).join('\n\n');
                          return (
