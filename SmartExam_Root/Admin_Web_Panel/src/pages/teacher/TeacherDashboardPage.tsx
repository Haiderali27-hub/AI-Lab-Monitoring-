...
export default function TeacherDashboardPage() {
  ...
  const handleDelete = async (exam: Exam) => {
    if (!window.confirm(`Delete exam "${exam.title}"? This removes its questions and student assignments.`)) return;
    setDeletingId(exam.examId);
    try {
      await examsApi.delete(exam.examId);
  ...
  return (
    <AppLayout title="Dashboard">
      <div className="max-w-7xl mx-auto space-y-8">
        {/* Greeting & Header */}
        <div>
          <h2 className="text-2xl font-bold text-slate-800 mb-1">{greeting}, {user?.name || firstName} 👋</h2>
                        ...
                        <div key={exam.examId} className="p-6 flex items-center justify-between hover:bg-slate-50/50 transition-all border-l-4 border-primary-500">
                          ...
                          <div className="ml-4 flex items-center gap-2">
                            <button
                              ...
                              className="px-4 py-2 bg-blue-50 hover:bg-blue-100 text-primary-600 text-xs font-bold rounded-lg transition-all"
                            >
                              Manage
                            </button>
                            <button
                              onClick={() => navigate(`/teacher/exams/${exam.examId}/edit`)}
                              className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-lg transition-all"
                              title="Edit exam details, questions and rules"
                            >
                              Edit
                            </button>
                            <button
                              ...
                              className="px-3 py-2 bg-red-50 hover:bg-red-100 text-red-600 text-xs font-bold rounded-lg transition-all disabled:opacity-50"
                              title="Delete this scheduled exam"
                            >
                              {deletingId === exam.examId ? '...' : 'Delete'}
                            </button>
                          </div>
                        </div>
                      ))
