...
export default function ExamAnalyticsPage() {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      // The backend returns { message, downloadUrl }. Derive the filename from it and
      // fetch with the bearer token (the endpoint is [Authorize]; a plain window.open
      // would send no Authorization header and get a 401).
      const fileName = (result.downloadUrl as string).split('/').pop() ?? '';
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {analytics.totalStudents === 0 ? (
                    ...
                    // The exam summary endpoint returns aggregate KPIs only (no per-student
                    // roster), so we do NOT fabricate candidate rows here. Honest empty state:
                    <tr>
                      <td colSpan={5} className="px-6 py-8">
                        <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-sm text-slate-500">
                          ...
                          <span>Detailed per-student leaderboard is not yet wired to live data (FYP-2).</span>
                        </div>
                      </td>
                    </tr>
                  )}
