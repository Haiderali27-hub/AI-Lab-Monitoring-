...
export default function TeacherSidebar() {
  ...
  const getLinkClass = ({ isActive }: { isActive: boolean }) => {
    const base = "flex items-center gap-3 font-medium mx-3 px-3 py-2.5 rounded-lg transition-all duration-200 ";
    const active = "bg-primary-50 text-primary-700 shadow-sm ";
    const inactive = "text-slate-500 hover:text-slate-800 hover:bg-slate-100/80 ";
    return base + (isActive ? active : inactive);
  };

  return (
    <aside className="w-[240px] h-screen bg-white border-r border-slate-200/70 shadow-sm flex flex-col py-8 shrink-0">
      {/* Brand Header */}
      <div className="px-5 mb-9 flex items-center gap-3">
        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-primary-500 to-primary-700 text-white flex items-center justify-center shadow-sm">
        ...
        <span className="text-lg font-bold text-slate-800 tracking-tight">SmartExam</span>
      </div>
      ...
      <div className="mt-auto px-4 pt-4 border-t border-slate-100 flex flex-col gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary-500 to-primary-700 text-white text-sm font-bold flex items-center justify-center shadow-sm">
