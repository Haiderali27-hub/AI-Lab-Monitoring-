    ...
    ILogger<AuthService> logger) : IAuthService
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            // Allow multiple organizations to be created
        ...
        {
            _logger.LogError(ex, "Organization registration failed");
            return ServiceResult<TokenResponse>.Fail("REGISTRATION_ERROR", $"Failed to create organization: {ex.Message}");
        }
    }
    ...
    public Task<bool> IsBootstrappedAsync(CancellationToken cancellationToken)
    {
        // In multi-tenant mode, the system is always "bootstrapped" (ready for org registration)
        // This endpoint now just indicates that the API is ready
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var departmentExists = await _dbContext.Departments.AnyAsync(
                x => x.Id == request.DepartmentId.Value && x.InstitutionId == institutionId && x.IsActive,
                cancellationToken);

            if (!departmentExists)
            {
                return ServiceResult<UserSummaryDto>.Fail("DEPARTMENT_NOT_FOUND", "Selected department was not found.");
            }
        ...
        if (request.SectionId.HasValue)
        {
            var section = await _dbContext.Sections
                ...
                .FirstOrDefaultAsync(
                    x => x.Id == request.SectionId.Value &&
            ...
            if (section is null)
            {
                return ServiceResult<UserSummaryDto>.Fail("SECTION_NOT_FOUND", "Selected section was not found.");
            }

            if (departmentId.HasValue && section.DepartmentId != departmentId.Value)
            {
                return ServiceResult<UserSummaryDto>.Fail("SECTION_DEPARTMENT_MISMATCH", "Selected section does not belong to the selected department.");
            }
        ...
        if (role == SystemRole.Student && sectionId.HasValue)
        {
            _dbContext.SectionEnrollments.Add(new SectionEnrollment
            {
        ...
        else if (role == SystemRole.Teacher && sectionId.HasValue)
        {
            _dbContext.TeacherSectionAssignments.Add(new TeacherSectionAssignment
            {
                SectionId = sectionId.Value,
                TeacherUser = user
            });
